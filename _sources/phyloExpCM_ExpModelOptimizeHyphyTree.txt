...
In both Equations :eq:`Frxy_FracTolerated` and :eq:`Frxy_HalpernBruno`, :math:`\omega` is a parameter that scales the rate of nonsynonymous versus synonymous mutations. A large value of :math:`\omega` means that nonsynonymous mutations are elevated relative to the expectation from the amino-acid preferences, and a small value means that they are depressed. Fixing :math:`\omega = 1` means that the amino-acid preferences naturally describe the relative rates of nonsynonymous and synonymous mutations by capturing all of the selection. This script allows models either with :math:`\omega = 1`, or :math:`\omega` as a free parameter. 

In both Equations :eq:`Frxy_FracTolerated` and :eq:`Frxy_HalpernBruno`, :math:`\beta` is a parameter that corresponds to the relative strength of selection the experiment used to determind :math:`\pi_{r,a}` versus natural evolution. The actual preference in nature for an amino-acid is taken to be proportional to :math:`\pi_{r,a}^{\beta}`. Therefore, values of :math:`\beta > 1` correspond to stronger selection in nature than in the experiments, and values of :math:`\beta < 1` correspond to weaker selection in nature than in the experiments. This script allows models with either the fixed value of :math:`\beta = 1` or with :math:`\beta` as a free parameter.

Note that the definitions in both Equations :eq:`Frxy_FracTolerated` and :eq:`Frxy_HalpernBruno` are `reversible`_ with respect to :math:`\left(\pi_{r,x}\right)^{\beta}`, such that

.. math::
   :label: Frxy_reversible

   \left(\pi_{r,x}\right)^{\beta} F_{r,xy} = \left(\pi_{r,y}\right)^{\beta} F_{r,yx}.
...
.. math::
   ...
   p_{r,x} &=& \frac{\left(\pi_{r,\mathcal{A}\left(x\right)}\right)^{\beta} \times q_x }{\sum\limits_y \left(\pi_{r,\mathcal{A}\left(y\right)}\right)^{\beta} \times q_y} \\
...
This matrix is symmetric since :math:`S_{r,xy} = S_{r,yx}` as can be verified from the fact that :math:`\frac{Q_{xy}}{q_y} = \frac{Q_{yx}}{q_x}` and :math:`\frac{F_{r,xy}}{\left(\pi_{r,\mathcal{A}\left(y\right)}\right)^{\beta}} = \frac{F_{r,yx}}{\left(\pi_{r,\mathcal{A}\left(x\right)}\right)^{\beta}}` as is guaranteed by the reversibility conditions in Equations :eq:`Qxy_reversible` and :eq:`Frxy_reversible`.
...
* *fitbeta* is an optional argument. If it is *None*, *False*, or is not specified, then the :math:`\beta` parameter defined in Equations :eq:`Frxy_FracTolerated` and :eq:`Frxy_HalpernBruno` is fixed to one. This corresponds to equal strength of selection in the experiments and natural evolution. If *fitbeta* is instead given a value of *freeparameter*, then the value of :math:`\beta` is fit by maximum likelihood, thereby allowing for weaker (:math:`\beta < 1`) or stronger (:math:`\beta > 1`) selection in nature than in the experiments.
...
The following primary output files are created. Each of these files begins with the prefix specified by *outfileprefix*, and then has the file name indicated.
    ...
    * ``optimizedtree_results.txt`` is a text file containing the results of optimizing the tree (this is the tree in ``optimizedtree.newick``). This file gives the log likelihood, the number of branch lengths optimized, the number of free model parameters, and the values of any free parameters. If *mutationrates* specifies known mutation rates then there are no free parameters. If *mutationrates* is set to *freeparameters*, then there are five mutation rate parameters as described above (description of reversible model in `Mutation rates`_ section), but since one of these mutation rates is not independent of the branch lengths, there are really on four free parameters (since we have :math:`R_{A \rightarrow C} = 1` as described above). Here are example contents of this file::
      ...
      If :math:`\beta` is also being fit via setting *fitbeta* to *freeparameter*, then the file also specifies the value of this parameter as in::
