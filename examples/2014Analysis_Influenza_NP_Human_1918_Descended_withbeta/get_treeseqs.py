...
viruses in the human lineage, including the H1N1 that re-emerged
in 1977. 

This is:

    * H1N1 from 1918 to 1957, and 1977 to 2008.
    
    * H2N2 from 1957 to 1968
    
    * H3N2 from 1968 to 2013. 

A maximum of 3 sequences per subtype per year are retained.
...
def main():
    ...
    outfile = 'Aligned_NPs.fasta' # output file
    outprotfile = 'Aligned_NP_proteins.fasta' # output file
    ...
    for ((strain, host, date, subtype), seq) in seqs:
        ...
        if key2 in hostretainedcounts:
            ...
            hostretainedcounts[key2] = 1
        hostcounts[host] += 1
        head = '%.2f_%d_%s_HOST_%s_%s' % (date, count, strain, host, subtype)
        head = head.replace(' ', '_')
        cleanseqs.append((head, seq))
        count += 1
    seqs = cleanseqs
